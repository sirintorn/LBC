...
//                        $scope.getGsdHeaderByCdt();
            ...
            $scope.onClickOkAs = function () {
                ...
                user_id = JSON.parse(localStorage.getItem("login_user_id"));
                    ...
                    if ($scope.cdt_dl_as.after_seq == 0 && $scope.cdt_dl_as.seq_max == 0) {
                        ...
                        $scope.upOldLbDAddNewLbD(after_seq, seq_max, emp_qty, lb_main_id, $scope.cdt_dl_as.workgroup_id, gcode_id);
                    }
                }

            }
            $scope.upOldLbDAddNewLbD = function (after_seq, seq_max, emp_qty, lb_main_id, workgroup_id, gcode_id) {
                var request = {after_seq: after_seq, seq_max: seq_max, emp_qty: emp_qty, lb_main_id: lb_main_id, workgroup_id: workgroup_id, gcode_id: gcode_id};
            ...
            $scope.addLbDetails = function (emp_qty, line_id, workgroup_id, gcode_id) {
                //                @FormParam("emp_qty") String emp_qty, @FormParam("lb_main_id") String lb_main_id
                var request = {emp_qty: emp_qty, lb_main_id: line_id, workgroup_id: workgroup_id, gcode_id: gcode_id};
...
//                $scope.cdt.total_sam = 0;
                $scope.cdt.takttime_plan = 60 / $scope.cdt.otp_target;
//                clearGraph();
//                $scope.getGsdHeaderByCust($scope.cdt.customer);
            }
...
//                        alert('2');
                        ...
                        var emp_qty = $scope.cdt_dl_as.emp_qty;
                        var lb_main_id = $scope.cdt.id;
                        $scope.upOldLbDAddNewLbD(after_seq, seq_max, emp_qty, lb_main_id);
                    }
                }

            }
            $scope.upOldLbDAddNewLbD = function (after_seq, seq_max, emp_qty, lb_main_id) {
                var request = {after_seq: after_seq, seq_max: seq_max, emp_qty: emp_qty, lb_main_id: lb_main_id};
                upOldLbDAddNewLbD(request, function (data) {
                    console.log("upOldLbDAddNewLbD", data);
                    if (null != data.updateLbDetailBySeq && data.updateLbDetailBySeq && null != data.addLbDetail && data.addLbDetail)
                    {
...
//                        $scope.cdt.factory_obj = {};
            ...
            function updateFormLbMain(data_obj) {
                ...
                var bn_eff = (((bn_output * $scope.cdt.total_sam) / (man * 60)) * 100).toFixed(3);
        ...
        .controller('GsdAnalysis', function ($state, $scope, $ionicPopup, $ionicModal, $ionicTabsDelegate, $ionicLoading) {
            ...
            //                var request = {customer: customer};
                            getGsdHeaderByCdt(request, function (data) {
                                ...
                                if (data.errorcode == "OK") {
                                    var data_obj = data.data[0];
                                    $scope.cdt.total_gsd_sam = (data_obj.total_sam - 0).toFixed(3);
                                } else {
                                    $scope.cdt.total_gsd_sam = 0;
                                }
                                hideLoading($ionicLoading);
            ...
            //                    console.log('$scope.cdt.ga_main_id', $scope.cdt.ga_main_id);
                        ...
                        function sumGsdCyclePercent(data_ar) {
                            ...
                            percent_gsd = ((total_cycletime - 0) - (total_gsd_sam - 0) * 100) / (total_gsd_sam - 0);

                            $scope.cdt.total_gsd_sam = Number(total_gsd_sam.toFixed(3));
                            $scope.cdt.total_cycletime = Number(total_cycletime.toFixed(3));
                            $scope.cdt.sam_detail_ratio = Number(((percent_gsd < 0 ? percent_gsd * (-1) : percent_gsd)).toFixed(3));
                        }
                        $scope.getGsdDataByCdn = function () {
                            showLoading($ionicLoading);
                            var request = {line_id: $scope.cdt.line_id, gsd_header_id: $scope.cdt.gsd_header_id};
            ...
            //                    var percent = (($scope.cdt_dl_timer.cycletime - $scope.cdt_dl_timer.gsd_sam) * 100) / $scope.cdt_dl_timer.gsd_sam;
                                var percent = $scope.cdt_dl_timer.gsd_sam / $scope.cdt_dl_timer.cycletime;
            ...
            //                    showLoading($ionicLoading);
                                ...
                                var request = {
                                    ...
                                    , diff: $scope.cdt_dl_timer.cycletime - $scope.cdt_dl_timer.sam_detail_ratio
            ...
            //                alert(row_data.sam_detail_ratio);
                            $scope.cdt_dl_timer.sam_detail_ratio = (row_data.sam_detail_ratio - 0).toFixed(3);
                            $scope.cdt_dl_timer.status = ($scope.cdt_dl_timer.cycletime > $scope.cdt_dl_timer.sam_detail_ratio ? "เกิน" : "ไม่เกิน");
